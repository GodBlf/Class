...
sort(arr.begin()+1, arr.end(), [](auto & a, auto & b) {
	return a < b;//这个是less函数 堆的是默认大根堆要反过来
	});

- int的是cmp函数 bool的是less函数因为stl堆默认大根堆所以要反过来 a>b
...
struct cmp {
	bool operator()(const int& a, const int& b) {
		return a > b;//堆的是默认大根堆要反过来 数组的是less函数
	}
};
priority_queue<int, vector<int>, cmp> buffer;
