...
## DI(依赖注入)
- 依赖注入解决的是手动构建和重复加载问题,框架自动梳理DAG图进行依赖排序一次性把变量全部加载
- 依赖注入是generator/pointer/callback的综合应用,简称gpc
- 应用变量生成器,传递指针实现注入,用框架决定生成唯一一个变量,解耦,解决了单例问题方便单元测试,不再需要单例模式;
- 底层通过依赖排序算法(拓扑排序算法),和回调函数实现
- workflow: type var pointer generator->callback
```go
package main
