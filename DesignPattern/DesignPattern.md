...
## 函数生成器
- 常用于回调函数的注册
- 解决的问题:回调函数通常被框架限制类型所以,通过函数生成器闭包的形式可以传递额外自定义信息
```go
func NewRegexFilter(pattern string) gin.HandlerFunc {
...
app := fx.Options(
		...
		fx.Invoke(func(a *A) {}),//触发
	)
app.Run() 
```

- 钩子函数(hook),就是管生命周期的回调函数
- 依赖注入框架:通过回调函数,实现依赖注入通过外部框架实现,框架自动梳理DAG图进行依赖排序,按顺序自动生成注入变量,这也叫IoC

# di
- DAG有向无环图
- 依赖注入:将type对象所需的依赖变量直接作为type成员写入type中,通过生成器new函数将依赖传入并生成对象,成员和对象并非"包含"关系
- 注入完type很自然的变量生成函数设置
- 依赖注入指针可以实现依赖的唯一生成,解决了单例模式难测试的问题

- 依赖注入框架:通过callback,实现依赖注入,对象生成交给外部框架管理,框架自动梳理DAG图进行依赖排序,按顺序自动生成注入变量,这也叫IoC
```go
package main

import "fmt"

type D struct{ Sync bool }
type C struct{ Conn string }
type B struct{ Repo *C }
type A struct{ Biz *B; CFG *D }

func NewD() *D { return &D{Sync: true} }

func NewC(d *D) *C { 
    return &C{Conn: "DB with Sync=" + fmt.Sprint(d.Sync)} 
}

func NewB(c *C) *B { return &B{Repo: c} }

func NewA(b *B, d *D) *A { return &A{Biz: b, CFG: d} }//传指针实现注入
...
func InitializeApp() *A {
    d := NewD()           // 1. 先造 D
    c := NewC(d)          // 2. 把 D 给 C
    b := NewB(c)          // 3. 把 C 给 B
    a := NewA(b, d)       // 4. 把 B 和 D 给 A
    return a
}
```
